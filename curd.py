...
import tracemalloc
import shutil
from sqlalchemy import or_,and_,func,Date
import csv
...
def Completed(db:Session,service_id:int,remarks:str):
...
def User_Wise_Day_Wise_Part_1(db: Session, picked_date: str, to_date: str):
    ...
    for row in db_res:
        ...
        data["type_of_activity"] = row.type_of_activity
        data["Nature of Work"] = row._nature_of_work
        data["gst_tan"] = row.gst_tan
        data["estimated_d_o_d"] =  row.estimated_d_o_d
        
        date_time2 = datetime.strptime(row.working_time, '%Y-%m-%d %H:%M:%S')
        time_diff = date_time1 - date_time2
        work_hour_hours_diff = time_diff.total_seconds() / 3600
        ...
        for row2 in db_res2:
            if row2.end_time_end and row2.end_time_start:
                ...
                end_hour_diff += time_diff.total_seconds() / 3600
        ...
        if row.work_status == "Reallocated":
            ...
            for row2 in db_res2:
                if row2.re_time_start and row2.re_time_end:
                    date_time2r = datetime.strptime(row2.re_time_start, '%Y-%m-%d %H:%M:%S')
                    re_time_diff = date_time1 - date_time2r
                    re_hour_diff += re_time_diff.total_seconds() / 3600
        ...
        for row2 in db_res2:
            if row2.hold_time_end and row2.hold_time_start:
                ...
                time_diff = date_time11 - date_time22
                hold_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in db_res2:
            if row2.meeting_time_end and row2.meeting_time_start:
                ...
                time_diff = date_time11 - date_time22
                meet_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in db_res2:
            if row2.break_time_end and row2.break_time_start:
                ...
                time_diff = date_time11 - date_time22
                break_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in db_res2:
            if row2.call_time_end and row2.call_time_start:
                ...
                time_diff = date_time11 - date_time22
                call_hour_diff += time_diff.total_seconds() / 3600

        data["call"] = call_hour_diff

        if row.work_status == "Completed":
            data["in_progress"] = 0
            data["completed"] = (a + b) - (call_hour_diff + break_hour_diff + meet_hour_diff + hold_hour_diff)
        else:
            data["completed"] = 0
            data["in_progress"] = (a + b) - (call_hour_diff + break_hour_diff + meet_hour_diff + hold_hour_diff)
...
#--------------------------------------------------------------------------------------------------------------------------------------
        ...
        for row2 in holdchargable:
            if row2.hold_time_end and row2.hold_time_start:
                ...
                date_time22 = datetime.strptime(row2.hold_time_start, '%Y-%m-%d %H:%M:%S')
                time_diff = date_time11 - date_time22
                holdchargable_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in Meetingchargable:
            if row2.meeting_time_end and row2.meeting_time_start:
                ...
                date_time22 = datetime.strptime(row2.meeting_time_start, '%Y-%m-%d %H:%M:%S')
                time_diff = date_time11 - date_time22
                meetch_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in Breakchargable:
            if row2.break_time_end and row2.break_time_start:
                ...
                date_time22 = datetime.strptime(row2.break_time_start, '%Y-%m-%d %H:%M:%S')
                time_diff = date_time11 - date_time22
                breakch_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in Callchargable:
            if row2.call_time_end and row2.call_time_start:
                ...
                date_time22 = datetime.strptime(row2.call_time_start, '%Y-%m-%d %H:%M:%S')
                time_diff = date_time11 - date_time22
                callch_hour_diff += time_diff.total_seconds() / 3600
        ...
        for row2 in endch:
            if row2.end_time_end and row2.end_time_start:
                ...
                date_time22 = datetime.strptime(row2.end_time_start, '%Y-%m-%d %H:%M:%S')
                time_diff = date_time11 - date_time22
                endch_hour_diff += time_diff.total_seconds() / 3600

        print(endch_hour_diff)
        ...
        if row.type_of_activity == "CHARGABLE":
            in_progress = a + b - (callch_hour_diff + breakch_hour_diff + meetch_hour_diff + holdchargable_hour_diff)

        data["chargable-time"] = endch_hour_diff -(callch_hour_diff+breakch_hour_diff+meetch_hour_diff+holdchargable_hour_diff)
...
#--------------------------------------------------------------------------------------------------------------------------------------
        ...
        data["Non-Charchable-time"] = endch_hour_diff -(callch_hour_diff+breakch_hour_diff+meetch_hour_diff+holdchargable_hour_diff)
